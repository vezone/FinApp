    ...
    class Tables
    {
        ...
        public (double number, string name) GetMostPopularProduct()
        {
            ...
            for (int i = 0; i < Content.Count; i++)
            {
                try
                {
                    var (numberPair, namePair) = Content[i].GetMostPopularProduct();
                    if (numberPair > number)
                    {
                        number = numberPair;
                        name = namePair;
                    }
            ...
            for (int i = 0; i < Content.Count; i++)
            {
                try
                {
                    var (cost, namePair) = Content[i].GetMostExpensiveProduct();
                }
                catch (System.FormatException ex)
                {
                    System.Console.WriteLine(ex.Message);
        ...
        public (double cost, string name) GetMostExpensiveProduct()
        {
            ...
            for (int i = 0; i < Content.Count; i++)
            {
                try
                {
                    var (costPair, namePair) = Content[i].GetMostExpensiveProduct();
                    if (costPair > cost)
                    {
                        cost = costPair;
                        name = namePair;
